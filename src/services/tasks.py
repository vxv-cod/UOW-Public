...
from db.unitofwork import ABCUnitOfWork


class TasksService:
    async def add_task(self, uow: ABCUnitOfWork, task: TaskSchemaAdd):
    ...
    async def get_tasks(self, uow: ABCUnitOfWork):
        async with uow:
            tasks = await uow.tasks.find_all()
            return tasks

    async def edit_task(self, uow: ABCUnitOfWork, task_id: int, task: TaskSchemaEdit):
        tasks_dict = task.model_dump()
        async with uow:
    ...
    async def get_task_history(self, uow: ABCUnitOfWork):
        async with uow:
